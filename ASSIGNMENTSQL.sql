...
select concat(first_name,' ',last_name) as name, department_id from employees where commission_pct is not null order by salary desc;
...
SELECT DEPARTMENT_ID, SUM(SALARY) as total_salary, SUM(COMMISSION_PCT) as t_commission FROM EMPLOYEES GROUP BY DEPARTMENT_ID;
...
SELECT DEPARTMENT_ID, SUM(SALARY) FROM EMPLOYEES GROUP BY EMPLOYEE_ID;  
...
-- earn the commission, then display as 'No commission'. Name the columns appropriately 
 ...
 SELECT CONCAT(FIRST_NAME,' ',LAST_NAME) AS EMPLOYEE_NAME,DEPARTMENT_ID, 
		IF (COMMISSION_PCT IS NULL , "No commission",Commission_pct)
						as earn_no_commission 
								from employees;
...
select first_name, last_name, commission_pct,
	case when commission_pct is not null then (2*commission_pct) else 'no_commission' end as new_commission from employees;
...
select e.first_name, e.department_id from employees e
											join employees e1 on e.first_name = e1.first_name
														and e.department_id = e1.department_id where e.employee_id <> e1.employee_id;
-- QES 9
...
SELECT concat(FIRST_NAME,' ',LAST_NAME) AS EMPLOYEE_NAME, DEPARTMENT_ID, SALARY FROM EMPLOYEES 
WHERE LAST_NAME LIKE '_A%' GROUP BY MANAGER_ID, EMPLOYEE_id, DEPARTMENT_ID;
...
select department_id, avg(sum(salary)) as avg_salary from employees group by department_id order by department_id;
...
select  commission_pct,
	case when commission_pct is not null then (0.1*salary) else 1 end as commission from employees;
...
select concat(first_name,'-',last_name) as employee_name, month(hire_date) as hire_month_of_emp from employees where 'employees_name' like '%a%';
...
select substring_index(substring_index(street_address,' ',2),' ',-1) as s_word from locations;
...
select first_name, lower(concat(left(first_name, 1),' ',last_name,' ','@systechusa.com')) as email_address from employees;
...
select concat(first_name,' ', last_name) as employee_name , min(salary) as lowest_salary from employees group by employee_name;
...
SELECT E.LAST_NAME, E.EMPLOYEE_ID,M.LAST_NAME,M.MANAGER_ID,N.LAST_NAME,N.EMPLOYEE_ID
       FROM EMPLOYEES AS E
				JOIN EMPLOYEES AS M USING (EMPLOYEE_ID)
                join EMPLOYEES AS N USING (EMPLOYEE_ID)
                       WHERE N.EMPLOYEE_ID = M.MANAGER_ID IS NULL;
                       -- DOUBTFULL
...
select EMP.HIRE_DATE AS EMP_HIRE_DATE,CONCAT(EMP.FIRST_NAME,' ',EMP.LAST_NAME) AS EMPLOYEE_NAME,MAN.HIRE_DATE AS MAN_HIRE_D,CONCAT(MAN.FIRST_NAME,' ',MAN.LAST_NAME) AS MANAGER_NAME
...
select e.last_name , e.hire_date from employees as e 
join departments as d
...
select last_name, department_id,job_id from employees where department_id = (select city from locations where city = 'ATLANTA');



SELECT E.LAST_NAME,D.DEPARTMENT_ID,E.JOB_ID ,L.CITY FROM EMPLOYEES E
										JOIN DEPARTMENTS D ON E.DEPARTMENT_ID = D.DEPARTMENT_ID
...
SELECT DEPARTMENT_ID , LAST_NAME , JOB_ID FROM EMPLOYEES 
       WHERE DEPARTMENT_ID = (SELECT DEPARTMENT_NAME FROM DEPARTMENTS 
												WHERE DEPARTMENT_NAME = 'OPERATIONS');


SELECT D.DEPARTMENT_NAME, E.JOB_ID ,E.LAST_NAME FROM DEPARTMENTS D
		JOIN EMPLOYEES E ON D.DEPARTMENT_ID = E.DEPARTMENT_ID
        WHERE D.DEPARTMENT_NAME =  'OPERATIONS';
        
select e.last_name , d.department_id, j.job_id from employees as e 
...
select employee_id, last_name,salary from employees 
	where salary > (select avg(salary) from employees)
			or department_id in (select department_id from employees where last_name like '%u%');
...
select concat(e.first_name,' ',e.last_name) as emp_name , j.job_title from employees as e 
join departments as d
on d.department_id = e.department_id
join jobs j on j.job_id = e.job_id  where department_name = 'sales';
...
SELECT LAST_NAME , SALARY, EMPLOYEE_ID FROM EMPLOYEES order by SALARY desc LIMIT 3;
...
select first_name, last_name, commission_pct,salary,
	case when commission_pct is null then '0' end from employees;


-- 12. Display the Managers (name) with top three salaries along with their salaries and department information.

SELECT E.MANAGER_ID, E.SALARY, D.DEPARTMENT_ID, D.DEPARTMENT_NAME 
FROM EMPLOYEES E
JOIN  DEPARTMENTS d
ON D.DEPARTMENT_ID = E.DEPARTMENT_ID
WHERE SALARY group by MANAGER_ID order by EMPLOYEE_ID DESC LIMIT 3;



SELECT  DEPARTMENT_ID , SALARY , MANAGER_ID , CONCAT(FIRST_NAME,' ',LAST_NAME) AS MANAGER_NAME FROM EMPLOYEES ORDER BY SALARY DESC LIMIT 3; 
